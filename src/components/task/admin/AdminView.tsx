...
const checkIsOverdue = (dateString: string, status: string) => {
...
const mapApiTaskToUI = (task: Task) => {
  const isOverdue = task.due_date
    ? checkIsOverdue(task.due_date, task.status)
    : false;
  ...
  return {
    ...
    assignedBy: "Super Admin", // Fallback if API doesn't provide creator name
    role: task.assigned_to_role || "Staff",
    location: task.location_name || "Downtown",
    isOverdue,
    dueDate: task.due_date
      ? new Date(task.due_date).toLocaleDateString("en-US", {
          year: "numeric",
...
export default function AdminView() {
  ...
  const [userToFire, setUserToFire] = useState<{
    name: string;
    email: string;
  } | null>(null);
  ...
  const handleConfirmReject = async (reason: string) => {
    if (!rejectTaskId) return;
    try {
      await rejectTask({ id: rejectTaskId, rejection_reason: reason }).unwrap();
      setIsRejectOpen(false);
    } catch (err) {
      console.error(err);
    }
  ...
  if (isLoading)
    return <div className="p-4 bg-black min-h-screen animate-pulse" />;
  if (error)
  ...
  return (
    <div className="space-y-8 p-4 bg-black min-h-screen text-white">
      ...
      <div className="space-y-6">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search tasks..."
            className="w-full bg-[#0D0D0D] border border-[#968B79]/40 rounded-2xl py-4 pl-12 pr-4 text-white focus:outline-none"
            value={searchQuery}
            onChange={(e) => handleSearchChange(e.target.value)}
          />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {taskStats.map((stat, i) => (
            <div
              key={i}
              className="bg-[#0A0A0A] border border-[#968B79]/30 rounded-3xl p-6 flex items-center justify-between"
            >
              <div>
                <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-1">
                  {stat.label}
      ...
      {/* Task Table */}
      <div className="hidden md:block bg-[#0A0A0A] border border-[#968B79]/30 rounded-[32px] overflow-hidden">
        <table className="w-full text-left">
          <thead>
            <tr className="text-gray-500 text-[10px] uppercase font-bold tracking-widest border-b border-[#1A1A1A]">
              <th className="px-6 py-5">Task</th>
              <th className="px-6 py-5">Assigned By</th>
              <th className="px-6 py-5">Due Date</th>
              <th className="px-6 py-5">Assigned To</th>
                ...
                >
                  ...
                  <td className="px-6 py-5">
                    <p
                      className={cn(
                        "text-xs font-bold",
                        task.isOverdue ? "text-red-500" : "text-gray-400",
                      )}
                    >
                      {task.dueDate}
                    </p>
                  ...
                  <td className="px-6 py-5 text-right">
                    <div className="flex justify-end gap-2">
                      {task.status === "Awaiting Review" ? (
                        <button
                          onClick={() => handleViewDetails(task)}
                          className="bg-[#1A1308] text-[#968B79] px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase hover:bg-[#2A1F0E]"
                        >
                          View
                        </button>
                      ) : task.isOverdue ? (
                        <button
                          ...
                          className="bg-red-900/20 text-red-500 px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase hover:bg-red-900/30"
                        >
                          Fire
                        </button>
                      ) : (
                        <div className="flex items-center gap-1">
                          ...
                          <button className="p-2 text-red-500/60 hover:text-red-500">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      )}
...
function StatusBadge({
  ...
  const styles: any = {
    ...
    "Awaiting Review": "bg-blue-500/10 text-blue-400 border-blue-500/20",
    Overdue: "bg-red-900/30 text-red-500 border-red-500/20",
  ...
  return (
    <span
      className={cn(
        "px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter border shrink-0",
